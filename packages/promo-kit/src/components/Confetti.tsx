...
// Deterministic confetti burst — the reward beat.
export const Confetti: React.FC<{ frame: number; start?: number; count?: number; life?: number }> = ({ frame, start = 0, count = 80, life = 130 }) => {
  const { width, height } = useVideoConfig();
  const c = useTheme().colors;
  const t = frame - start;
  if (t < 0 || t > life + 20) return null;

  const palette = [c.accent, c.primary, c.gold, c.accentSoft, c.primarySoft, c.goldSoft];
  const cx = width / 2;
  ...
  return (
    <AbsoluteFill style={{ pointerEvents: "none" }}>
      {pieces.map((p, i) => (
        <div key={i} style={{ position: "absolute", left: p.x, top: p.y, width: p.w, height: p.round ? p.w : p.h, background: p.col, borderRadius: p.round ? "50%" : 2, opacity: p.op, transform: `rotate(${p.rot}deg)` }} />
      ))}
    </AbsoluteFill>
  );
};
