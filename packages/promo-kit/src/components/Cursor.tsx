...
// Cartoon hand-cursor that travels and presses real UI. `pressed` 0→1 scales it down like a click.
export const Cursor: React.FC<{ x: number; y: number; scale?: number; pressed?: number; rotation?: number }> = ({ x, y, scale = 1, pressed = 0, rotation = -8 }) => {
  ...
  return (
    <div
      style={{
        ...
        filter: `drop-shadow(0 8px 14px ${alpha(c.ink, 0.28)})`,
        pointerEvents: "none",
      }}
    >
      ...
      {pressed > 0.02 ? (
        <div
          style={{
            ...
            marginTop: -20 * scale,
            borderRadius: "50%",
            border: `3px solid ${alpha(c.primary, 0.6 * (1 - pressed))}`,
