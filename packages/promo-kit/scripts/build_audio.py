#!/usr/bin/env python3
"""
Distribution adaptation: the FX timeline can now be supplied as JSON
(`--fx cues.json`, a list of {effect, t, vol}) instead of being edited in this
file, and `--out` / `--sfx-dir` make the paths explicit so a worker can run it
against a per-project directory. Behaviour with no new flags is unchanged.
...
def main():
    ...
    ap.add_argument("--fx", help='JSON file: [{"effect","t","vol"}] replacing the built-in FX timeline')
    ap.add_argument("--out", help="output WAV path (default public/audio/master.wav)")
    ap.add_argument("--sfx-dir", help="directory of sound effect files (default public/sfx)")
    args = ap.parse_args()
    dur = args.duration

    # Distribution adaptation: cues as data, paths as arguments.
    global FX, SFX, MASTER, OUT_DIR
    if args.sfx_dir:
        SFX = os.path.abspath(os.path.expanduser(args.sfx_dir))
    if args.out:
        MASTER = os.path.abspath(os.path.expanduser(args.out))
        OUT_DIR = os.path.dirname(MASTER)
    if args.fx:
        with open(args.fx) as fh:
            cues = json.load(fh)
        FX = [(c["effect"], float(c["t"]), float(c.get("vol", 0.8))) for c in cues]
