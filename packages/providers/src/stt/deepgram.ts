...
import { isRetryableStatus, backoffSecs } from "../policy";
...
export class DeepgramProvider implements SttProvider {
  ...
  async transcribe(wavPath: string, durationSec: number, opts: SttOptions) {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      const res = await fetch(url, { method: "POST", headers: { authorization: `Token ${key}`, "content-type": "audio/wav" }, body: audio, signal: opts.signal ?? null });
      if (res.ok) {
        const json = (await res.json()) as unknown;
        opts.onProgress?.(90, "normalising transcript");
        const minutes = durationSec / 60;
        await opts.recordUsage?.({ provider: "deepgram", model, seconds: durationSec, usdEstimate: minutes * (STT_PRICE_PER_MIN["deepgram:nova-2"] ?? 0) });
