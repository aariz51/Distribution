...
export class WhisperApiProvider implements SttProvider {
  ...
  async transcribe(wavPath: string, durationSec: number, opts: SttOptions) {
    ...
    for (const [i, c] of chunks.entries()) {
      ...
      const json = await this.upload(t, c.file, opts);
    ...
    await opts.recordUsage?.({ provider: t.name, model: t.model, seconds: durationSec, usdEstimate: (durationSec / 60) * t.price });
    return NormalizedTranscript.parse({ language, duration: durationSec, speakers: [], words, segments });
  }

  private async upload(t: NonNullable<ReturnType<WhisperApiProvider["target"]>>, file: string, opts: SttOptions): Promise<VerboseJson> {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      const res = await fetch(t.url, { method: "POST", headers: { authorization: `Bearer ${t.key}` }, body: form, signal: opts.signal ?? null });
      if (res.ok) return (await res.json()) as VerboseJson;
      ...
      await new Promise((r) => setTimeout(r, backoffSecs(attempt, res.headers.get("retry-after")) * 1000));
    }
    throw lastErr;
  }
}
