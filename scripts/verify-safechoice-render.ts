...
async function main() {
  ...
  const built = buildStoryboard({ profile, screens, logo: "logo/app-logo.png", durationSec: 18 });
  if (built.storyboard.scenes.some(scene => "score" in scene.copy)) throw new Error("Director fabricated a score");
  const outputDir = path.resolve("storage/tmp/qa-verified-safechoice");
  ...
  const outPath = path.join(outputDir, "promo-vertical.mp4");
  let last = -10;
  await renderPromo({ storyboard: built.storyboard, theme: themeForProduct(profile), compositionId: "PromoVertical", publicDir: staged, outPath, concurrency: 1, onProgress: pct => {
    if (pct >= last + 10) { console.log(`Rendering ${pct}%`); last = pct; }
  }});
  const probe = await probeMedia(outPath);
  if (probe.width !== 1080 || probe.height !== 1920 || !probe.hasAudio || Math.abs(probe.durationSec - 18) > 0.15) throw new Error("Output media contract failed");
  await run(bin("ffmpeg"), ["-v", "error", "-xerror", "-i", outPath, "-f", "null", "-"], { timeoutMs: 120_000 });
  const proof = built.storyboard.scenes.find(scene => scene.kind === "dashboard")!;
  const framePath = path.join(outputDir, "proof.png");
