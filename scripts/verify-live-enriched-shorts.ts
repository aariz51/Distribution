...
try {
 ...
 const work=await db.select().from(jobs).where(eq(jobs.projectId,projectId));assert(work.some(j=>j.id==='0c05c01c-c41f-4d0f-ab15-07f6ecd25fdd' && j.status==='completed'));
 ...
 assert.equal(clips.length,2);assert.equal(enriched.length,3);assert.equal(covers.length,6);
 ...
 for(const asset of [...clips,...enriched,...covers]){
  ...
  if(asset.type==='clip_enriched'){
   assert.equal(asset.metadata.voice,'female');assert.deepEqual(asset.metadata.steps,asset.id==='7a937cbc-0cfa-4103-b064-d589d7d47f09'?['broll','sfx','outro']:['outro']);
 ...
 console.log('PASS: original and finished screening proofs, two clips/three female-voice derivatives including full enrichment/six covers, full decode, attribution and authenticated delivery');
