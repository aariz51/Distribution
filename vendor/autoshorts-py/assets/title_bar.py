...
MIN_FONT = 34
# Where the title starts, as a fraction of height. Phones overlay a notch,
# a clock and the app's own controls across the top ~8%, so the banner begins
# below that rather than fighting it.
TOP_SAFE_FRAC = 0.095
...
def render_banner(text: str, width: int, height: int, limit_y: int) -> "object":
    ...
    # Clear of the phone's own furniture. At 2.5% the banner sat under the
    # notch and the app's top bar on TikTok and Reels, so the first line was
    # partly hidden on a real handset even though it looked fine in the file.
    top_y = int(height * TOP_SAFE_FRAC)
